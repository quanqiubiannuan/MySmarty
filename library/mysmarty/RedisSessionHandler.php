...
class RedisSessionHandler implements \SessionHandlerInterface
{
    ...
    {
        $db = config('session.session_type_params.redis.db', 0);
        return Redis::getInstance()->select($db);
    }
    ...
    {
        $data = Redis::getInstance()->get($session_id);
        if (!empty($data)) {
            return $data;
        }
        return '';
    ...
    public function write($session_id, $session_data)
    {
        ...
        Redis::getInstance()->set($session_id, $session_data);
        if ($lifetime > 0) {
            Redis::getInstance()->expire($session_id, $lifetime);
        }
        return true;
    }
    ...
    {
        Redis::getInstance()->del($session_id);
        return true;
    }
