...
class MysqlSessionHandler implements \SessionHandlerInterface
{
    ...
    {
        $this->database = config('session.session_type_params.mysql.database');
        $this->table = config('session.session_type_params.mysql.table');
        return true;
    }
    ...
    {
        $id = Model::getInstance()->name($this->database)
            ->table($this->table)
            ->add([
    ...
    {
        $min_create_time = time() - $maxlifetime;
        Model::getInstance()->name($this->database)
            ->table($this->table)
