                ...
                if (!empty($match[2]) && !isset($smarty->ext->$name)) {
                    ...
                    if (!class_exists($class)) {
                        ...
                        if (!isset($this->resolvedProperties[$match[0]][$objType])) {
                            $property = isset($this->resolvedProperties['property'][$basename]) ? $this->resolvedProperties['property'][$basename] : $property = $this->resolvedProperties['property'][$basename] = strtolower(join('_', preg_split('/([A-Z][^A-Z]*)/', $basename, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE)));
                            if ($property !== false) {
